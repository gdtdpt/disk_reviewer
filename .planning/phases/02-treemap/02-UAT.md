...
expected: |
  ...
  矩形使用 squarified 算法，长宽比接近 1:1（不会出现全部水平排列的情况）。
  点击响应流畅，无卡顿。
...
### 4. 双击下钻进入子目录
expected: 双击目录对应的矩形色块，进入该子目录的 Treemap 视图。单击仅选中色块，不触发下钻。
result: [pending]
...
### 7. 选中项高亮
expected: 单击某个矩形色块后，该色块显示高亮边框（金色描边）。
result: [pending]
...
### 14. 垂直渐变填充
expected: 每个色块从上到下显示垂直渐变效果（顶部 60% 纯色基色，底部 40% 渐变为白色）。
result: [pending]
