...
Plans:
- [ ] 01-01: 项目初始化 + Rust 工程脚手架（Cargo.toml、依赖配置、模块结构）
- [ ] 01-02: 逻辑盘枚举（GetLogicalDrives + GetDiskFreeSpaceExW）
- [ ] 01-03: 异步目录遍历器（FindFirstFileExW + 线程池 + 增量推送）
- [ ] 01-04: 扫描结果数据结构 + 无权限目录处理 + Others 聚合
...
Plans:
- [ ] 02-01: egui 应用框架 + 主窗口布局
- [ ] 02-02: Squarified Treemap 布局算法
- [ ] 02-03: Treemap egui 渲染（矩形、标签、颜色映射）
- [ ] 02-04: 下钻交互 + 面包屑导航
- [ ] 02-05: 详情面板 + 选中高亮
...
Plans:
- [ ] 03-01: SQLite 快照存储（schema 设计 + 读写）
- [ ] 03-02: 快照序列化/反序列化 + 快照管理 UI
- [ ] 03-03: 差异检测算法（树结构对比）
- [ ] 03-04: 差异高亮渲染 + 快照对比视图
