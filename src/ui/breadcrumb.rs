...
pub fn breadcrumb_ui(
    ...
    egui::ScrollArea::horizontal().show(ui, |ui| {
        ui.horizontal(|ui| {
            // 根节点按钮
            if ui.add(egui::Button::new(egui::RichText::new(&scan_result.name).monospace().size(14.0))).clicked() {
            ...
            for (depth, &idx) in nav_stack.iter().enumerate() {
                ...
                if let Some(crate::scanner::Entry::Dir(dir)) = current.children.get(idx) {
                    if ui.add(egui::Button::new(egui::RichText::new(&dir.name).monospace().size(14.0))).clicked() {
